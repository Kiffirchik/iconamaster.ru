...
export function ConsultationLinks({ iconTitle, compact = false, includeViewing = false }) {
  const { bundle } = useContent();
  const contacts = bundle?.contacts ?? {};
  const links = buildContactLinks(contacts, iconTitle);
    ...
    <div className={`consultation-links${compact ? ' consultation-links--compact' : ''}`}>
      {links.whatsapp ? (
        <a className="button button--primary" href={links.whatsapp} target="_blank" rel="noreferrer">
          {iconTitle ? 'Получить консультацию об иконе' : 'Написать в WhatsApp'}
        </a>
      ) : null}
      {viewing?.whatsapp ? (
