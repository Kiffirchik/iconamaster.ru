...
$fields=array();
foreach(array('title','price','discount','newPrice','availability','description','moreDetails','size','period','purpose','technique','condition','expertise') as $key) $fields[$key]=isset($row[$key]) ? (string)$row[$key] : '';
...
$data['fields']['price']='100 000 руб.'; $data['fields']['discount']='10'; $data['fields']['newPrice']='90 000';
$data['fields']['moreDetails']="Дополнительный текст\n\nБезопасный <b>текст</b>";
$r=request($path,$data);
...
verify(strpos($r[1],'<summary>Подробнее об иконе</summary>')!==false && strpos($r[1],"Дополнительный текст\n\nБезопасный &lt;b&gt;текст&lt;/b&gt;")!==false,'saved multiline details appear safely in HTTP response');
