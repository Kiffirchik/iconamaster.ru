...
test "$release" = new-icons-wedding-20260912
for digest in "$payload_sha" "$migration_sha" "$additions_sha"; do
...
test "$(sha256sum "$base/append-wedding-pair-20260912.php" | cut -d ' ' -f 1)" = "$migration_sha"
test "$(sha256sum "$base/wedding-pair-20260912.json" | cut -d ' ' -f 1)" = "$additions_sha"
...
/usr/local/bin/php52 "$base/append-wedding-pair-20260912.php" "$stage" "$base/wedding-pair-20260912.json"
