#!/bin/sh
# Narrow, checksummed overlay. All editable data except videos.json remains live.
set -eu
test "$#" -eq 1
payload_sha=$1
...
for file in "$live"/content/*.json; do
  name=$(basename "$file")
  if test "$name" != videos.json; then cmp "$file" "$stage/content/$name"; fi
done
