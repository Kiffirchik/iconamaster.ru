...
if (!$root || (strpos(basename($root),'iconamaster.ru.stage-discounts-')!==0 && strpos(basename($root),'editor-test-')!==0)) die("Private stage directory required\n");
...
foreach($rows as $index=>$row) {
    foreach(array('discount','newPrice') as $key) if (!array_key_exists($key,$row)) { $rows[$index][$key]=null; $added++; }
    $check=$rows[$index];
    foreach(array('discount','newPrice') as $key) if (!array_key_exists($key,$before[$index])) unset($check[$key]);
...
if ($added) {
    ...
    $temp=tempnam($root.'/content','.discounts-');
...
echo 'Migrated '.count($rows).' icons; added '.$added." nullable fields; existing data preserved\n";
