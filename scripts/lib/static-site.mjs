...
function renderMetrika(metrikaId) {
  ...
  function pageview(){
    ...
    ym(${metrikaId},'hit',url,{title:document.title,referer:lastUrl||document.referrer||''});
    lastUrl=url;
  }
  function start(){
  ...
  ym(${metrikaId},'init',{defer:true,clickmap:true,trackLinks:true,accurateTrackBounce:true,webvisor:false});
  pageview();
  }
  window.addEventListener('iconamaster:pageview',pageview);
